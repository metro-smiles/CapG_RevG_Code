...
class GRUModel_ContinueStop_Text(nn.Module):
	def __init__(self, hidden_size, output_size, max_star, nos_imgfeat, star_op_size, cont_flag, n_layers_cont, n_layers_text):
	...
	def forward(self, input, hidden, flag = -1):
		if flag != -1: # Passing image features and stars for the first GRU of every sentence - Sentence RNN
		...
		else: # Passing word embeddings - Word RNN
